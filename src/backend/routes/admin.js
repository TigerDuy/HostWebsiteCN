...
router.get("/recipes", verifyAdminOrModerator(db), async (req, res) => {
  try {
...
router.get("/users", verifyAdminOrModerator(db), async (req, res) => {
  try {
    const result = await db.query(
...
router.delete("/delete/:id", verifyAdmin(db), async (req, res) => {
  const recipeId = req.params.id;
  try {
    await db.query("DELETE FROM cong_thuc WHERE id = $1", [recipeId]);
...
router.delete("/user/:id", verifyAdmin(db), async (req, res) => {
  const userId = req.params.id;
  try {
    await db.query("DELETE FROM nguoi_dung WHERE id = $1", [userId]);
...
router.put("/user/:id/role", verifyAdmin(db), async (req, res) => {
  const userId = req.params.id;
  ...
  try {
    ...
    const userRows = await db.query(
      "SELECT role, password_reset_at, password FROM nguoi_dung WHERE id = $1",
      [userId]
    );
    ...
    if (currentRole === "admin" && (role === "moderator" || role === "user")) {
      ...
      // Kiểm tra nếu password vừa được reset (trong 10 phút)
      if (passwordResetAt) {
        const resetTime = new Date(passwordResetAt).getTime();
        const now = new Date().getTime();
        const timeDiff = (now - resetTime) / (1000 * 60);
    ...
    async function updateRole() {
      await db.query(
        "UPDATE nguoi_dung SET role = $1, password_reset_at = NULL WHERE id = $2",
        [role, userId]
      );
    ...
    res.status(500).json({ message: "❌ Lỗi đổi vai trò!" });
  }
});

// ✅ API reset password người dùng (Admin)
const mailer = require('../config/mailer');
router.post("/user/:id/reset-password", verifyAdmin(db), async (req, res) => {
  const userId = req.params.id;
  ...
  try {
    ...
    const rows = await db.query("SELECT email, username FROM nguoi_dung WHERE id = $1", [userId]);
    ...
    const result = await db.query(
      "UPDATE nguoi_dung SET password = $1, password_reset_at = NOW() WHERE id = $2",
      [hashed, userId]
    );
