...
(async () => {
  ...
  try {
    ...
    if (!rows || rows.length === 0) {
      console.log('❌ No user found with email "admin@gmail.com". Nothing to change.');
      process.exit(0);
    }
    ...
    if (user.role === 'admin') {
      console.log('⚠️  User "' + user.username + '" already has role "admin". No change needed.');
      process.exit(0);
    }

    // Create backup
    const ts = new Date().toISOString().replace(/[:.]/g, '-');
    const backupPath = path.join(backupDir, `admin_gmail_role_change_backup_${ts}.json`);
    ...
    console.log(`✅ Updated user "${user.username}" (id=${user.id}): ${user.role} → admin. affectedRows=${res.affectedRows}`);
